...
FLwsSendBuffer::FLwsSendBuffer(const uint8* Data, SIZE_T Size, bool bInIsBinary)
	: bIsBinary(bInIsBinary), BytesWritten(0), bHasError(false)
{
...
FLwsWebSocket::FLwsWebSocket(const FString& InUrl, const TArray<FString>& InProtocols, const FString& InUpgradeHeader)
	: State(EState::None), LastGameThreadState(EState::None), bWasSendQueueEmpty(true), LwsConnection(nullptr), 
	Url(InUrl), Protocols(InProtocols), UpgradeHeader(InUpgradeHeader), Identifier(++IncrementingIdentifier)
{
...
void FLwsWebSocket::Send(const void* Data, SIZE_T Size, bool bIsBinary)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("My Socket push at %d.%d"), date.ToUnixTimestamp(), date.GetMillisecond());
}
...
void FLwsWebSocket::SendFromQueue()
{
	...
	if (SendQueue.Peek(CurrentBuffer))
	{
		...
		auto date = FDateTime::Now();
		UE_LOG(LogTemp, Warning, TEXT("Socket send at %d.%d"), date.ToUnixTimestamp(), date.GetMillisecond());
...
bool FLwsWebSocket::WriteBuffer(FLwsSendBuffer& Buffer)
{
	...
	auto date = FDateTime::Now();
	UE_LOG(LogTemp, Warning, TEXT("LWS send at %d.%d"), date.ToUnixTimestamp(), date.GetMillisecond());
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
	{
		...
		if (bWantsMessageEvents)
		{
			...
			if (BytesLeft == 0)
			{
				...
				FLwsReceiveBufferTextPtr BufferText = MakeUnique<FLwsReceiveBufferText>(MoveTemp(ReceiveBuffer));
				ReceiveBuffer.Empty();
				auto date = FDateTime::Now();
				UE_LOG(LogTemp, Warning, TEXT("My Socket get at %d.%d"), date.ToUnixTimestamp(), date.GetMillisecond());
